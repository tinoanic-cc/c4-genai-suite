...
@Extension()
export class AzureOpenAIModelExtension implements Extension {
  ...
  getMiddlewares(_: User, configuration: AzureOpenAIModelExtensionConfiguration): Promise<ChatMiddleware[]> {
    const middleware = {
      invoke: async (context: ChatContext, getContext: GetContext, next: ChatNextDelegate): Promise<any> => {
        context.llms[this.spec.name] = await context.cache.get(this.spec.name, configuration, () => {
          ...
          const callbacks = [getEstimatedUsageCallback('azure-open-ai', configuration.deploymentName, getContext)];
