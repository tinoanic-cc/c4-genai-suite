...
# libre office needs libreoffice to be installed on the server
...
@lru_cache
def get_format_providers(config: Config):
    return [cls(config=config) for cls in get_all_subclasses(AbstractFormatProvider) if cls(config=config).enabled]  # type: ignore


@lru_cache
def get_format_provider_mappings(config: Config):
